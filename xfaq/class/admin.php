...
class ModuleAdmin
{
    ...
    private $_itemLabel = array();
    private $_itemLineLabel = array();
    private $_itemConfigLabel = '';
    private $_itemLineConfigLabel = array();
    private $_itemChangelogLabel = '';
    private $_obj = array();
    ...
    function __construct()
    {
        ...
        echo"<style type=\"text/css\" media=\"screen\">@import \"" . XOOPS_URL
            . "/modules/" . $xoopsModule->getVar('dirname', 'e')
            . "/css/admin.css\";</style>";
    ...
    function renderButton($position = "right", $delimeter = "&nbsp;")
    {
        ...
        foreach (
            array_keys($this->_itemButton) as $i
        ) {
            $ret .= "<a class='ui-corner-all tooltip' href='"
                . $this->_itemButton[$i]['link'] . "' title='"
                . $this->_itemButton[$i]['title'] . "'>";
            $ret .= "<img src='" . $path . $this->_itemButton[$i]['icon']
                . "' title='" . $this->_itemButton[$i]['title'] . "' />"
                . $this->_itemButton[$i]['title']
                . $this->_itemButton[$i]['extra'];
            $ret .= "</a>\n";
            $ret .= $delimeter;
        }
        $ret .= "</div>\n</div>\n";
    ...
    )
    {
        ...
        switch ($type) {
            ...
            case "module":
                ...
                $line
                    .= "<a href=\"http://" . $this->_obj->getInfo('license_url')
                    . "\" target=\"_blank\" >" . $this->_obj->getInfo('license')
                    . "</a>\n";
                $line .= "<br />\n";

                $line .= "<a href=\"http://" . $this->_obj->getInfo('website')
                    . "\" target=\"_blank\" >" . $this->_obj->getInfo('website')
                    . "</a>\n";
                $line .= "<br />\n";
                if ($value != '') {
                    $line
    ...
    function renderLabel()
    {
        $ret = "";
        foreach (
            array_keys($this->_itemLabel) as $i
        ) {
            $ret .= "<fieldset><legend class=\"label\">";
            $ret .= $this->_itemLabel[$i]['title'];
            $ret .= "</legend>\n";
            foreach (
                array_keys($this->_itemLineLabel) as $k
            ) {
                if ($this->_itemLineLabel[$k]['label']
                    == $this->_itemLabel[$i]['title']
                ) {
                ...
                if (phpversion() < $value) {
                    $line
                        ...
                        "<span style='color : red; font-weight : bold;'><img src='"
                            . $path . "off.png' >"
                            . sprintf($text, $value, phpversion())
                            . "</span>\n";
                } else {
                    $line .= "<span style='color : green;'><img src='" . $path
                        . "on.png' >" . sprintf($text, $value, phpversion())
                        . "</span>\n";
                }
                break;
            ...
            case "xoops":
                if (substr(XOOPS_VERSION, 6, strlen(XOOPS_VERSION) - 6) < $value
                    ...
                    $line
                        .=
                        "<span style='color : red; font-weight : bold;'><img src='"
                            . $path . "off.png' >" . sprintf(
                            $text, $value, substr(
                                XOOPS_VERSION, 6, strlen(XOOPS_VERSION) - 6
                            )
                        ) . "</span>\n";
                } else {
                    $line .= "<span style='color : green;'><img src='" . $path
            ...
            case "folder":
                if (!is_dir($value)) {
                    $line
                        .=
                        "<span style='color : red; font-weight : bold;'><img src='"
                            . $path . "off.png' >" . sprintf($text[1], $value)
                            . "</span>\n";
                } else {
                    $line .= "<span style='color : green;'><img src='" . $path
                        . "on.png' >" . sprintf($text[0], $value) . "</span>\n";
                }
                break;
                ...
                if (is_dir($value[0])) {
                    if (substr(decoct(fileperms($value[0])), 2) != $value[1]) {
                        $line
                            ...
                            "<span style='color : red; font-weight : bold;'><img src='"
                                . $path . "off.png' >"
                                . sprintf($text, $value[0], $value[1], substr(decoct(fileperms($value[0])), 2))
                                . "</span>\n";
                    } else {
                        $line
                            .= "<span style='color : green;'><img src='" . $path
                            . "on.png' >"
                            . sprintf($text, $value[0], $value[1], substr(decoct(fileperms($value[0])), 2))
                            . "</span>\n";
                    }
    ...
    function addChangelogLabel($title)
    {
        ...
        if (!is_file(
            XOOPS_ROOT_PATH . "/modules/" . $this->_obj->getVar("dirname")
                . "/docs/changelog.txt"
        )
        ) {
            $language = 'english';
        }
        $language = empty($language) ? $GLOBALS['xoopsConfig']['language']
            : $language;
        $file = XOOPS_ROOT_PATH . "/modules/" . $this->_obj->getVar("dirname")
            . "/docs/changelog.txt";
        if (is_readable($file)) {
    ...
    function addNavigation($menu = '')
    {
        ...
        foreach (
            array_keys($this->_obj->adminmenu) as $i
        ) {
            if ($this->_obj->adminmenu[$i]['link'] == "admin/" . $menu) {
                $ret
                    = "<div class=\"CPbigTitle\" style=\"background-image: url("
                    . $path . $this->_obj->adminmenu[$i]['icon'] . "); background-repeat: no-repeat; background-position: left; padding-left: 50px;\">
                        <strong>" . $this->_obj->adminmenu[$i]['title']
                    . "</strong></div><br />";
            }
        }
        return $ret;
        ...
        foreach (
            ...
            if ($this->_obj->adminmenu[$i]['link'] != 'admin/index.php') {
                if (isset($this->_obj->adminmenu[$i]['menu'])) {
                    $ret .= "<a href=\"../" . $this->_obj->adminmenu[$i]['link']
                        . "\" title=\"" . $this->_obj->adminmenu[$i]['title']
                        . "\">" . "<img src=\"" . $path
                        . $this->_obj->adminmenu[$i]['menu'] . "\" alt=\""
                        . $this->_obj->adminmenu[$i]['title'] . "\" />";
                } else {
                    $ret .= "<a href=\"../" . $this->_obj->adminmenu[$i]['link']
                        . "\" title=\"" . $this->_obj->adminmenu[$i]['title']
                        . "\">" . "<img src=\"" . $path
                        . $this->_obj->adminmenu[$i]['icon'] . "\" alt=\""
                        . $this->_obj->adminmenu[$i]['title'] . "\" />";
                }
                $ret
                    .=
                    "<span>" . $this->_obj->adminmenu[$i]['title'] . "</span>";
                $ret .= "</a>";
            }
        }
        if ($this->_obj->getInfo('help')) {
            $ret .= "<a href=\"" . $pathsystem . "help.php?mid="
                . $this->_obj->getVar('mid', 's') . "&amp;"
                . $this->_obj->getInfo('help') . "\" title=\"" . _AM_SYSTEM_HELP
                . "\">" . "<img width=\"32px\" src=\"" . $path
                . "images/admin/help.png\" alt=\"" . _AM_SYSTEM_HELP . "\" /> ";
    ...
    function renderIndex()
    {
        ...
        if ($this->_itemConfigLabel != '') {
            ...
            $ret .= "</legend><br/>\n";
            foreach (
                array_keys($this->_itemLineConfigLabel) as $i
            ) {
